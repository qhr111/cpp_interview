...
class memPool{
public:
    memPool(int wkNum): wkNum(wkNum){}
    ~memPool(){
        stop();
    }
    void start(){
        for(int i = 0; i < wkNum; i++){
            workers.emplace_back(&loop, this);
        }
    }
    void loop(){
        while(true){
...
// A minimal fixed-size block memory pool suitable for whiteboard/onsite coding.
// Not thread-safe; keeps a freelist of blocks carved from larger chunks.
class MemoryPool {
public:
	explicit MemoryPool(std::size_t blockSize, std::size_t blocksPerChunk = 1024)
		: blockSize_(std::max(blockSize, sizeof(FreeNode))), blocksPerChunk_(blocksPerChunk) {
		addChunk();
	}
...
private:
	...
	void addChunk() {
		std::size_t bytes = blockSize_ * blocksPerChunk_;
		char *chunk = static_cast<char *>(::operator new(bytes));
		chunks_.push_back(chunk);

		// Link all blocks in the new chunk into the freelist.
...
String::String(String &&other) 　　// 移动构造函数的参数不能为const
{  
     m_data = other.data;
     other.data = nullptr;
}
...
void* memcpy(void* dest, const void* src, size_t len) {
    ...
    // 按8字节块拷贝（使用uint64_t）
    uint64_t* d8 = static_cast<uint64_t*>(dest);
    const uint64_t* s8 = static_cast<const uint64_t*>(src);
    size_t count8 = len / 8;
    ...
    // 拷贝剩余字节
    char* d1 = reinterpret_cast<char*>(d8 + count8);
    const char* s1 = reinterpret_cast<const char*>(s8 + count8);
...
const char* strstr(const char* str1, const char* str2) {
    ...
    const char* tmp_s = str1;
    
    while (*tmp_s != '\0') {
        const char* s1 = tmp_s;
        const char* s2 = str2;
        
        // 同时比较，直到某个字符串结束或字符不相等
